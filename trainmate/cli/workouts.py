...
def run_workout_adapt(args: argparse.Namespace) -> None:
    ...
    try:
        reason, proposed_workouts = cli.coach_service.adapt(date_str)
        ...
        if confirm == 'y':
            ...
            cli.coach_service.apply_adaptations(
...
def run_workout_generate(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        reasoning, workouts = cli.coach_service.generate_workouts(**workout_kwargs)
...
def run_workout_swap(args: argparse.Namespace) -> None:
    ...
    warnings = cli.coach_service.validate_swap(ops)
    if warnings:
        print(bold(yellow("\nSwap warnings:")))
        for msg in warnings:
            print(yellow(f"  - {msg}"))
    ...
    updated = cli.coach_service.apply_swap(ops, args.no_sync, reason=args.reason)
