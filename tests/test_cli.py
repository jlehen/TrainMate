...
class TestTrainMateCLI(unittest.TestCase):
    ...
    def test_plan_commands(self, mock_coach, mock_garmin):
        ...
        mock_coach.generate_periodization_plan.assert_called_once_with(force=False, auto_apply=False)

        mock_coach.generate_periodization_plan.reset_mock()
        exit_code, stdout, stderr = self.run_cli(["plan", "generate", "-f"])
        self.assertEqual(exit_code, 0)
        mock_coach.generate_periodization_plan.assert_called_once_with(force=True, auto_apply=False)
