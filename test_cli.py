...
class TestTrainMateCLI(unittest.TestCase):
    ...
    def test_workout_commands(self, mock_coach, mock_sheets_reader):
        ...
        mock_coach.replan.assert_called_once_with(force=False)

        # 2b. Replan workouts with force
        mock_coach.replan.reset_mock()
        exit_code, stdout, stderr = self.run_cli(['workout', 'plan', '-f'])
        self.assertEqual(exit_code, 0)
        mock_coach.replan.assert_called_once_with(force=True)

        mock_coach.replan.reset_mock()
        exit_code, stdout, stderr = self.run_cli(['workout', 'plan', '--force'])
        self.assertEqual(exit_code, 0)
        mock_coach.replan.assert_called_once_with(force=True)
